...
void MS5611_ReadAll(MS5611_t *dev, int32_t *temperature, int32_t *pressure,
		int32_t *altitude) {
	dev_ctx = *dev;
	...
	if (TEMP < 2000) {
		int32_t T2 = (dT * dT) >> 31;
		int64_t OFF2 = 5 * ((TEMP - 2000LL) * (TEMP - 2000LL)) >> 1;
		int64_t SENS2 = 5 * ((TEMP - 2000LL) * (TEMP - 2000LL)) >> 2;
		TEMP -= T2;
		OFF -= OFF2;
		SENS -= SENS2;
	}
	...
	float pressure_hPa = P / 100.0f;
	float h = 44330.0f * (1.0f - powf(pressure_hPa / 1013.25f, 0.1903f)) * 100.0f;
